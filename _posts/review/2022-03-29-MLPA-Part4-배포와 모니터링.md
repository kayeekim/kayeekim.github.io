...
* 서버 측 배포 server-side deployment
* 클라이언트 측 배포
* 연합 학습: 하이브리드 방법

### 9.1. 서버 측 배포 server-side deployment
-. server-side deployment 는 client 로부터 요청을 받아 추론 파이프라인을 실행하여 결과를 반환하는 웹 서버로 구성
...
-. 예측을 계산하는 데 **필요한 정보를 미리 얻을 수 있는 경우** & 모델의 요청을 도착하는 대로 처리하는 것이 아닌 **한번에 많은 양의 요청을 처리할 수 있는 경우** 배치 방식이 적합.


### 9.1.1 스트리밍 애플리케이션 또는 API (스트리밍 방식)
-. 스트리밍 방식: **모델을** 사용자가 요청을 보낼수 있는 **엔드포인트 end-point 로 간주**
